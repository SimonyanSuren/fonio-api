...
import { AccountNumberReq, DurationTypes } from '../../util/swagger';
...
export class AccountNumberFacade {
  ...
  async getTrackingNumbers(
    ...
    let query = this.entityManager
      .createQueryBuilder(AccountNumber, 'an')
    ...
    if (filter && filter.trim()) {
      query.andWhere(
        ' (lower(an.poolName) like lower(:filter) OR LOWER (an.comp_name) LIKE LOWER (:filter) OR lower(an.visitorFrom) like lower(:filter) OR an.number::varchar like :filter OR an.destination_number::varchar like :filter )  ',
      );
  ...
  async getTrackingNumberBy(userID, companyID, search) {
    ...
    return this.entityManager.query(
      `select * from tracking_numbers where company_id=$1 and user_id=$2 AND number::text like '${s}%' OR pool_name like '${search}%' OR destination_number::text like '${s}%'`,
      [companyID, userID],
    );
  }
  ...
  async getUserAccountTrackingNumbers(userID, companyID) {
    return (
      await this.entityManager.query(
        `SELECT ARRAY (SELECT number FROM tracking_numbers WHERE company_id=${companyID} AND user_id=${userID})`,
      )
    )[0].array;
  }
  ...
  async delete(id, currentUser) {
    ...
    return await manager.transaction(async (tEM) => {
      let trackingNumber = await this.getTrackingNumberById(
        id,
        currentUser.userId,
        currentUser.companyId,
      );
      if (!trackingNumber)
        throw new Error('tracking_number:thisTrackingNumberDoesNotExist');
      ...
      if (did && did.didOpentactIdentityID)
        await this.opentactService.releaseAppDid(
          token.token,
  ...
  async disableEnable(id, currentUser, status) {
    ...
    if (!trackingNumber)
      throw new Error('tracking_number:thisTrackingNumberDoesNotExist');
    return await this.disableEnableDb(
      id,
      currentUser.userId,
      currentUser.companyId,
      status,
  ...
  async isMoreThenOneField(body) {
    ...
    if (arr.length > 1)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassOneFromNextFields(alwaysSwap,direct,lendParams,lendPage,webRef,search)',
      );
    return arr;
  }

  async getObjectForSavingTrackingNumberPatch(
    body,
    userID,
    companyID,
    trackingNumberEntityOld,
    numberField,
    isCountCorrect,
  ) {
    ...
    return {
      ...
      recordCalls: body.recordCalls
        ? body.recordCalls
        : trackingNumberEntityOld.recordCalls,
      recordCallsBoolean: body.recordCallsBoolean
        ? body.recordCallsBoolean
        : trackingNumberEntityOld.recordCallsBoolean,
      whisperMessage: body.whisperMessage
        ? body.whisperMessage
        : trackingNumberEntityOld.whisperMessage,
      whisperMessageBoolean: body.whisperMessageBoolean
        ? body.whisperMessageBoolean
        : trackingNumberEntityOld.whisperMessageBoolean,
      number: numberField ? numberField : body.number,
      visitorFor:
        isCountCorrect && isCountCorrect.length > 0
          ? body.search
            ? body.visitorFor
            : null
          : body.visitorFor
          ? trackingNumberEntityOld.search
            ? body.visitorFor
            : await HelperClass.throwErrorHelper(
                'did:youCanNotChangeFieldvisitorForBecauseYourCurrentFieldSearchAreNotTrue',
              )
          : trackingNumberEntityOld.visitorFor,
      visitorFrom: body.visitorFrom
        ? body.visitorFrom
        : trackingNumberEntityOld.visitorFrom,
      alwaysSwap:
        body.alwaysSwap != undefined && body.alwaysSwap != null
          ? body.alwaysSwap
          : trackingNumberEntityOld.alwaysSwap,
      direct:
        body.direct != undefined && body.direct != null
          ? body.direct
          : trackingNumberEntityOld.direct,
      lendParams:
        body.lendParams != undefined && body.lendParams != null
          ? body.lendParams
          : trackingNumberEntityOld.lendParams,
      lendPage:
        body.lendPage != undefined && body.lendPage != null
          ? body.lendPage
          : trackingNumberEntityOld.lendPage,
      webRef:
        body.webRef != undefined && body.webRef != null
          ? body.webRef
          : trackingNumberEntityOld.webRef,
      search:
      ...
      poolSize: body.poolSize
        ? body.poolSize
        : trackingNumberEntityOld.poolSize,
      destinationNumber: body.destinationNumber
        ? body.destinationNumber
        : trackingNumberEntityOld.destinationNumber,
      poolName: body.poolName
        ? body.poolName
        : trackingNumberEntityOld.poolName,
      trackCampaign:
        body.trackCampaign || body.trackEachVisitor
          ? body.trackCampaign &&
            trackingNumberEntityOld.trackCampaign !== body.trackCampaign
            ? !trackingNumberEntityOld.trackCampaign
            : trackingNumberEntityOld.trackCampaign
          : trackingNumberEntityOld.trackCampaign,
      trackEachVisitor:
        body.trackCampaign || body.trackEachVisitor
          ? body.trackEachVisitor &&
            trackingNumberEntityOld.trackEachVisitor !== body.trackEachVisitor
            ? !trackingNumberEntityOld.trackEachVisitor
            : trackingNumberEntityOld.trackEachVisitor
          : trackingNumberEntityOld.trackEachVisitor,
      numberOnWebSite: body.numberOnWebSite
        ? body.numberOnWebSite
        : trackingNumberEntityOld.numberOnWebSite,
      numberOnline: body.numberOnline
        ? body.numberOnline
        : trackingNumberEntityOld.numberOnline,
      companyUuid: body.companyUuid
        ? body.companyUUid
        : trackingNumberEntityOld.companyUuid,
      isTextMessaging:
        body.isTextMessaging != undefined && body.isTextMessaging != null
          ? body.isTextMessaging
          : trackingNumberEntityOld.isTextMessaging,
      callFlow: callFlow,
    };
  }
  ...
  async patch(currentUser, body) {
    try {
      if (!body.id)
        await HelperClass.throwErrorHelper('tracking_number:youShouldPassId');
      let trackingNumber: any = await this.getTrackingNumberById(
        body.id,
        currentUser.userId,
        currentUser.companyId,
      );
      if (!trackingNumber)
        await HelperClass.throwErrorHelper(
          'tracking_number:thisTrackingNumberDoesNotExist',
        );
      let token = await this.opentactAuth.adminLoginGettignToken();
      ...
      let didEntity = body.didID
        ? {
            isDidPassed: true,
            isDidExist: await this.isDidExistInReturnedList(
              didEntitiesList,
              body.didID,
            ),
          }
      ...
      if (didEntity.isDidPassed && !didEntity.isDidExist)
        await HelperClass.throwErrorHelper(
          `did:thisDidIsNotExist[${body.didID}]`,
        );
      let plan = body.planID ? await this.getPlanByID(body.planID) : null;
      if (body.planID && !plan)
        await HelperClass.throwErrorHelper(`did:planDoesNotExist`);
      let booleanFields: any =
        body.alwaysSwap ||
        body.direct ||
        body.lendParams ||
        body.lendPage ||
      ...
      if (
        booleanFields.isSomeFieldPassed &&
        booleanFields.isCountCorrect[0] === 'search' &&
        !body.visitorFrom &&
        !body.visitorFor
      )
        await HelperClass.throwErrorHelper(
          `tracking_number: youShouldPAssVisitorsFromAndvisitorForIfYouPassSearchParameter`,
        );
      if (
        booleanFields.isSomeFieldPassed &&
        booleanFields.isCountCorrect[0] === 'webRef' &&
        (!body.visitorFrom || body.visitorFor)
      )
        await HelperClass.throwErrorHelper(
          `tracking_number: youShouldPAssVisitorsFromFieldIfYouPassWebRefParamAndYouShouldNotPassVisitorFor`,
        );
      if (
        booleanFields.isSomeFieldPassed &&
        booleanFields.isCountCorrect[0] === 'lendPage' &&
        (!body.visitorFrom || body.visitorFor)
      )
        await HelperClass.throwErrorHelper(
          `tracking_number: youShouldPAssVisitorsFromFieldIfYouPassLendPageParamAndYouShouldNotPassVisitorFor`,
        );
      if (
        booleanFields.isSomeFieldPassed &&
        booleanFields.isCountCorrect[0] === 'lendParams' &&
        (!body.visitorFrom || body.visitorFor)
      )
        await HelperClass.throwErrorHelper(
          `tracking_number: youShouldPAssVisitorsFromFieldIfYouPassLendParamsParamAndYouShouldNotPassVisitorFor`,
        );
      let whisperMessage =
        body.whisperMessageBoolean || body.whisperMessage
          ? {
              isWhisperMessageBooleanPassed: body.whisperMessageBoolean,
              isWhisperMessagePassed: body.whisperMessage,
              isCorrectFirst: body.whisperMessageBoolean
                ? !body.whisperMessage
                  ? await HelperClass.throwErrorHelper(
                      'tracking_number:yourShouldPassWhisperMessageTextFieldBooleanFieldSelected',
                    )
                  : true
                : true,
            }
      ...
      if (body.trackEachVisitor && body.trackCampaign)
        await HelperClass.throwErrorHelper(
          `did:ifYouWantToChangeOneFromThisFields[trackCampaign,trackEachVisitor]youShouldChooseOnlyOneFromThem`,
        );
      let company: any = body.companyUuid
        ? await this.companyFacade.getCompanyByUuid(body.companyUuid)
        : trackingNumber.companyUuid;
      if (body.companyUuid && !company)
        await HelperClass.throwErrorHelper('did:thisCompanyDoesNotExist');
      let object = await this.getObjectForSavingTrackingNumberPatch(
        body,
        currentUser.userId,
        currentUser.companyId,
        trackingNumber,
        body.number,
        booleanFields.isCountCorrect,
      );
  ...
  async create(currentUser, accountNumber: AccountNumberReq) {
    ...
    let plan = accountNumber.planID
      ? await this.getPlanByID(accountNumber.planID)
      : null;
    if (accountNumber.planID && !plan)
      await HelperClass.throwErrorHelper(
        'tracking_number:thisPlanDoesNotExist',
      );
    let trackingNumber = await this.isDidNumberAlreadyAddedInTrackingTable(
      didID,
      currentUser.userId,
    );
    if (trackingNumber)
      await HelperClass.throwErrorHelper(
        `tracking_number: thisNumber(${trackingNumber.number}),alreadyAdded`,
      );
    if (!accountNumber.numberOnline)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassNumberOnlineField',
      );
    if (!accountNumber.numberOnWebSite)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassNumberOnWebSiteField',
      );
    if (!accountNumber.trackEachVisitor && !accountNumber.trackCampaign)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassOneFieldOrTrackEachVisitorOrTrackCampaign',
      );
    if (accountNumber.trackEachVisitor && accountNumber.trackCampaign)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassOneFieldOrTrackEachVisitorOrTrackCampaign',
      );
    if (!accountNumber.poolName)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassPoolNameField',
      );
    if (!accountNumber.destinationNumber)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassDestinationNumberField',
      );
    if (!accountNumber.poolSize)
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassPoolSizeField',
      );
    if (
      !accountNumber.search &&
      !accountNumber.webRef &&
      !accountNumber.lendPage &&
      !accountNumber.lendParams &&
      !accountNumber.direct &&
      !accountNumber.alwaysSwap
    )
      await HelperClass.throwErrorHelper(
        'tracking_number:youShouldPassOneFromNextFields(alwaysSwap,direct,lendParams,lendPage,webRef,search)',
      );
    await this.isMoreThenOneField(accountNumber);
    let search = accountNumber.search
      ? !accountNumber.visitorFrom
        ? await HelperClass.throwErrorHelper(`
    tracking_number: youShouldPAssVisitorsFromField
`)
        : !accountNumber.visitorFor
        ? await HelperClass.throwErrorHelper(`
    tracking_number: youShouldPAssVisitorForField
`)
        : {
            visitorFor: accountNumber.visitorFor,
            visitorsFrom: accountNumber.visitorFrom,
          }
      : null;
    let webRef = accountNumber.webRef
      ? !accountNumber.visitorFrom
        ? await HelperClass.throwErrorHelper(`
    tracking_number: youShouldPAssVisitorsFromField
`)
...
`)
        : true
      : null;
    let landingPage = accountNumber.lendPage
      ? !accountNumber.visitorFrom
        ? await HelperClass.throwErrorHelper(`
    tracking_number: youShouldPAssVisitorsFromField
`)
...
`)
        : true
      : null;
    let landingParams = accountNumber.lendParams
      ? !accountNumber.visitorFrom
        ? await HelperClass.throwErrorHelper(`
    tracking_number: youShouldPAssVisitorsFromField
`)
...
`)
        : true
      : null;
    let whisperMessage = accountNumber.whisperMessageBoolean
      ? !accountNumber.whisperMessage
        ? await HelperClass.throwErrorHelper(
            'tracking_number:yourShouldPassWhisperMessageTextFieldBooleanFieldSelected',
          )
        : accountNumber.whisperMessage
    ...
    let identity = await this.opentactService.getIdentity(
      token.token,
      currentUser.userUuid,
    );
    let response = await this.opentactService.assignDidToIdentity(
      token.token,
      undefined,
      /*callStatusUrl,*/ entity.id,
      identity.id,
    );
    let didNumber = response.did.number;
    let did = await this.didFacade.addDidAfterBuy(
      userId,
      companyId,
      entity.did.id,
      true,
  ...
  getObjectForSavingTrackingNumber(
    ...
    return {
      ...
      whisperMessage: accountNumber.whisperMessage
        ? accountNumber.whisperMessage
        : '',
      whisperMessageBoolean: accountNumber.whisperMessageBoolean
        ? accountNumber.whisperMessageBoolean
        : false,
      number: didNumber,
      ...
      destinationNumber: accountNumber.destinationNumber
        ? accountNumber.destinationNumber
        : 1,
      poolName: accountNumber.poolName ? accountNumber.poolName : '',
      trackCampaign: accountNumber.trackCampaign
        ? accountNumber.trackCampaign
        : false,
      trackEachVisitor: accountNumber.trackEachVisitor
        ? accountNumber.trackEachVisitor
        : false,
      numberOnWebSite: accountNumber.numberOnWebSite
        ? accountNumber.numberOnWebSite
        : false,
      numberOnline: accountNumber.numberOnline
        ? accountNumber.numberOnline
        : false,
      companyUuid: accountNumber.companyUuid,
      companyName: companyName,
      isTextMessaging: accountNumber.isTextMessaging
        ? accountNumber.isTextMessaging
        : false,
    };
  }
  ...
  isDidNumberAlreadyAddedInTrackingTable(didID, userID) {
    if (typeof didID == 'number') {
      ...
      typeof didID == 'string' &&
      didID.indexOf('op2_' + Constants.VENDOR_FOR_INTELIQUENT) === 0
    ) {
  ...
  async findById(companyId: number, acnuId: number) {
    let manager = await this.entityManager;
    return manager
      .createQueryBuilder(AccountNumber, 'acnu')
  ...
  async assignCallFlow(currentUser, id, caflId) {
    let callFlow = new CallFlow();
    callFlow.id = caflId;
    const accountNumber = await this.entityManager
      .createQueryBuilder()
      .update(AccountNumber)
      .set({
        callFlow: callFlow,
  ...
  async countTrackingNumberPrice(
    trackingNumbers: { tn: string; features: string[] }[],
    durationUnit: string,
    duration: number,
  ) {
    const pricingTable = await this.entityManager.query(
      'SELECT * from pricing',
    );

    const { tf_number_nrc, tf_number_monthly_mrc, tf_number_yearly_mrc } =
      pricingTable[0];
      ...
      if (didsAlreadyBuyed.includes(number)) {
        price +=
          durationUnit === DurationTypes.month_duration
            ...
            : duration * tf_number_yearly_mrc;
      } else {
        price +=
          durationUnit === DurationTypes.month_duration
            ? duration * tf_number_monthly_mrc + tf_number_nrc
            : duration * tf_number_yearly_mrc + tf_number_nrc;
      }
      pricesResult[number] = price;
      price = 0;
    }
