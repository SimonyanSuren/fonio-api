import { Company, Contact, User } from '../../models';
...
export class ContactFacade {
  ...
  async findById(compId: number, contId: number) {
    ...
    return manager
      .createQueryBuilder(Contact, 'cont')
  ...
  async findByCompId(compId: number, options: any) {
    ...
    if (options?.firstName) {
      query.andWhere('LOWER(cont.firstName) like :name', {
  ...
  async create(currentUser, contact_req: ContactReq) {
    ...
    if (!currentUser.companyUuid) {
      await HelperClass.throwErrorHelper('user:thisUserIsNotAssinedToCompany');
    }

    let company = await this.companyFacade.getCompanyByUuid(
      currentUser.companyUuid,
    );

    if (!company) {
      await HelperClass.throwErrorHelper('company:CompanyDoesNotExist');
    }
    ...
    const savedContact = await this.entityManager.save(contact);
  ...
  async edit(currentUser, id, contact_req: ContactReq) {
    ...
    if (!company)
      ...
      if (!contact)
        await HelperClass.throwErrorHelper('contact:ContactDoNotExist');
      else {
        contact.phoneNumber = contact_req.phoneNumber;
        contact.firstName = contact_req.firstName;
        contact.lastName = contact_req.lastName;
  ...
  async delete(currentUser, id) {
    ...
    if (!company)
      ...
      return manager.query(
        'delete from contacts where comp_id=$1 and cont_id=$2',
