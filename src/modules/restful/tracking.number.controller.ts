...
export class TrackingNumberController {
  ...
  @ApiResponse({
    status: 200,
    description: 'numbers trackingNumber OK',
    type: AccountNumber,
    isArray: false,
  })
  ...
  @ApiQuery({
    ...
    name: 'search',
    description:
      'Default: "xxxx". 4 (for "long_code") or 7 (for "toll_free") digit numbers. To search full number provide the whole 4 or 7 numbers. In a case of seraching numbers that are starting or ending with provideed ones, fill the missing ones with "x". For example: xx67, 67xx or xxxx123, 123xxxxx.',
  })
  @ApiQuery({
    name: 'take',
    description: 'Max elements to return. default 10',
  ...
  public async getBySearch(
    ...
    @Body('country') country: string = 'US',
    @Body('city') city: any,
    @Body('state') state: any,
    @Body('rate_center') rate_center: any,
    @Body('type') type: any = 'long_code',
    ...
    try {
      if (!['toll_free', 'long_code'].includes(type)) {
        return res.status(HttpStatus.BAD_REQUEST).json({
          message:
            "type must me one of the following ['toll_free', 'long_code']",
        });
      }
      let num_type: any =
        type === 'long_code'
          ? { line: search || 'xxxx' }
          : { pattern: search || 'xxxxxxx' };
      let response = await this.opentactService.getTrackingNumbers({
        ...{
          type,
          take,
  ...
  public async disableEnable(@Req() req, @Res() res: Response) {
    try {
      ...
      let response = await this.accountNumberFacade.disableEnable(
        id,
        req.user,
        status,
      );
  ...
  public async assignCallFlow(
    ...
    try {
      let response = await this.accountNumberFacade.assignCallFlow(
        req.user,
        id,
        caflId,
      );
  ...
  public async getProvinces(
    ...
    try {
      let response = await this.commonService.getEntities(
        this.Repositories.PROVINCE,
        {
          page,
          pageSize,
          where: {},
