...
export class AuthController {
  ...
  public async getTokens(@Req() req, @Res() res: Response) {
    ...
    let response = await this.authService.getEntity(
  ...
  @ApiResponse({
    status: 200,
    description: 'Reset Password OK, so the mail has not been found.',
  })
  @Post('reset/password')
  public async resetPassword(@Req() req: Request, @Res() res: Response) {
    try {
      ...
      if (!email)
        await HelperClass.throwErrorHelper('email:youShouldPassEmail');
  ...
  public async setPassword(@Req() req, @Res() res: Response) {
    try {
      ...
      if (!password)
        await HelperClass.throwErrorHelper('auth:youShouldPassPassword');
      let user: object | any = await this.userFacade.findByHash(key);
      if (!user)
        await HelperClass.throwErrorHelper('auth:invalidVerificationCode');
      const equals = await await comparePassword(
        password,
        user.password ? user.password : '',
      );
      if (equals)
        await HelperClass.throwErrorHelper(
          'user:youCanNotUseTheSamePasswordTryNewOne',
        );
      ...
      if (!user.emailConfirmed)
        await HelperClass.throwErrorHelper('user:disabled');
      ...
      let userToken = await AuthService.generateToken(
        user,
        remoteAddress,
        userAgent,
        user.salt || '',
  ...
  public async generateApiKey(@Req() req, @Res() res: Response) {
    try {
      ...
      const api_key: any = await this.authService.generateApiKey(
        req.user,
        remoteAddress,
        userAgent,
      );
